...
export function createSocketClient(
  ...
  coveyTownID: string,
    ...
    privateChatReceived: Promise<Record<string, string>>;
    publicChatReceived: Promise<Record<string, string>>;
  } {
  ...
  const privateChatPromise = new Promise<Record<string, string>>(resolve => {
    socket.on('chatMsgReceivedPrivate', (message: Record<string, string>) => {
      resolve(message);
    });
  });
  const publicChatPromise = new Promise<Record<string, string>>(resolve => {
    socket.on('chatMsgReceivedPublic', (message: Record<string, string>) => {
      resolve(message);
    });
  });
  ...
  return {
    ...
    privateChatReceived: privateChatPromise,
    publicChatReceived: publicChatPromise,
  };
}
